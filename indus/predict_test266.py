"""Two-hundred-and-sixty-sixth registered prediction set (PREDICTIONS.md, GW1-GW4): decipherment loop 91, three more
grammar rules for lines parse11 leaves unparsed, each scored by the G margin (grammar.margin), A and B.
  COUNT-3     a numeral run, then exactly 3 lexical signs (COUNT allows at most 2)
  HEAD-COUNT  the heading unit (817 / 820 / 861 + 2 / 60 / 1), then a numeral run and 0-2 lexical signs
  CAGE-MID    1+ lexical signs, a caged sign, then a rest that parses as a name
Writes results/predict_test266.md."""
...
def main():
    ...
    rd = R.Round('Two-hundred-and-sixty-sixth registered predictions: decipherment loop 91, three more grammar rules', 'predict_test266')
    ...
    rules = {
        'COUNT-3': lambda t: count_n(t, 0, 3, 3),
        'HEAD-COUNT': lambda t: len(t) >= 3 and t[0] in ('817', '820', '861') and t[1] in ('2', '60', '1') and count_n(t, 2, 0, 2),
        'CAGE-MID': lambda t: any(t[k] in CAGED and all(lexical(g) for g in t[:k]) and p8(t[k + 1:]) in ('name', 'formula', 'bare') for k in range(1, len(t) - 2)),
    }
    keep = []
    for k, (name, f) in enumerate(rules.items()):
        ...
        rd.rec('GW%d' % (k + 1), '%s raises the margin on A by 0.3 point or more and on B' % name, 'A %.2f -> %.2f; B %.2f -> %.2f' % (100 * mA, 100 * a, 100 * mB, 100 * b), ok)
        if ok:
            keep.append(f)
    g = lambda t: base(t) or any(f(tuple(t)) for f in keep)
    a, b = margin(DL, g), margin(DB, g)
    rd.rec('GW4', 'progress rule: the held rules together raise the margin on A (0.3+) and B', 'A %.2f -> %.2f; B %.2f -> %.2f (%d rules)' % (100 * mA, 100 * a, 100 * mB, 100 * b, len(keep)), bool(keep) and a - mA >= 0.003 and b > mB)
    rd.finish()
