"""Three-hundred-and-eighteenth registered prediction set (PREDICTIONS.md, G91-G93): decipherment loop 143, two more
grammar rules for lines parse11 leaves unparsed, each scored by the G margin (grammar.margin), A and B.
  BODY-90   3+ lexical signs, then 90 (as BODY-400, set 263)
  SHORT-90  1-2 lexical signs, then 90
Writes results/predict_test318.md."""
...
def main():
    ...
    rd = R.Round('Three-hundred-and-eighteenth registered predictions: decipherment loop 143, two more grammar rules', 'predict_test318')
    ...
    rules = {
        'BODY-90': lambda t: len(t) >= 4 and t[-1] == '90' and all(lexical(g) for g in t[:-1]),
        'SHORT-90': lambda t: 2 <= len(t) <= 3 and t[-1] == '90' and all(lexical(g) for g in t[:-1]),
    }
    keep = []
    for k, (name, f) in enumerate(rules.items()):
        ...
        rd.rec('G9%d' % (k + 1), '%s raises the margin on A by 0.3 point or more and on B' % name, 'A %.2f -> %.2f; B %.2f -> %.2f' % (100 * mA, 100 * a, 100 * mB, 100 * b), ok)
        if ok:
            keep.append(f)
    g = lambda t: base(t) or any(f(tuple(t)) for f in keep)
    a, b = margin(DL, g), margin(DB, g)
    rd.rec('G93', 'progress rule: the held rules together raise the margin on A (0.3+) and B', 'A %.2f -> %.2f; B %.2f -> %.2f (%d rules)' % (100 * mA, 100 * a, 100 * mB, 100 * b, len(keep)), bool(keep) and a - mA >= 0.003 and b > mB)
    rd.finish()
