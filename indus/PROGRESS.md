...
| 3 | U vault | share of tokens in texts never used to fix the values (B's new lines; later, newly excavated finds) read as sense with values fixed beforehand | 0 (no sound value has passed); picture vault (set 201): shared phrases predict unseen tablets' pictures 24.5% / 18.6% (nulls p = 0.001 / 0.03), but a third split fails (10.3%, p = 0.16; set 202): split-dependent |
