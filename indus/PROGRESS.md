...
| 2 | C positive-control gate | a method proposing sound values or readings must first recover a known answer (Linear B -> Greek). Methods that passed / were run | 2 of 4 passed: substitution-phonetics (set 179), character-LM key scorer (set 205); key bench failed (Ventris's Linear B key beaten by 11-41 of 100 shuffles, `linb_control.md`); rebus search not run |
