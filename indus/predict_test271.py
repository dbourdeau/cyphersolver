"""Two-hundred-and-seventy-first registered prediction set (PREDICTIONS.md, GY1-GY4): decipherment loop 96, three more
grammar rules for lines parse11 leaves unparsed, each scored by the G margin (grammar.margin), A and B.
  HH-STRICT  exactly two signs, both strict heads (head 5+ A names, more often head than modifier)
  OPEN-H     exactly two signs, an opening-prone sign then a strict head
  H-LOW      exactly two signs, a strict head then a low-head-propensity sign (LOW-POST's set)
Writes results/predict_test271.md."""
...
def main():
    ...
    rd = R.Round('Two-hundred-and-seventy-first registered predictions: decipherment loop 96, three more grammar rules', 'predict_test271')
    ...
    rules = {
        'HH-STRICT': lambda t: len(t) == 2 and t[0] in H and t[1] in H,
        'OPEN-H': lambda t: len(t) == 2 and t[0] in openp and t[1] in H,
        'H-LOW': lambda t: len(t) == 2 and t[0] in H and t[1] in low,
    }
    keep = []
    for k, (name, f) in enumerate(rules.items()):
        ...
        rd.rec('GY%d' % (k + 1), '%s raises the margin on A by 0.3 point or more and on B' % name, 'A %.2f -> %.2f; B %.2f -> %.2f' % (100 * mA, 100 * a, 100 * mB, 100 * b), ok)
        if ok:
            keep.append(f)
    g = lambda t: base(t) or any(f(tuple(t)) for f in keep)
    a, b = margin(DL, g), margin(DB, g)
    rd.rec('GY4', 'progress rule: the held rules together raise the margin on A (0.3+) and B', 'A %.2f -> %.2f; B %.2f -> %.2f (%d rules)' % (100 * mA, 100 * a, 100 * mB, 100 * b, len(keep)), bool(keep) and a - mA >= 0.003 and b > mB)
    rd.finish()
