"""Three-hundred-and-thirty-fourth registered prediction set (PREDICTIONS.md, SX1-SX4): decipherment loop 159, the
combined referent pool stratified by site and object class together (seal, tablet, tag): the stratum of set 333 becomes
(site, class). A unit's expected
...
the current ones if lower. Writes results/predict_test334.md."""
import random
...
def cls(ty):
    return 'tab' if ty.startswith('TAB') else 'tag' if ty.startswith('TAG') else 'seal'
...
def main():
    ...
    rd = R.Round('Three-hundred-and-thirty-fourth registered predictions: decipherment loop 159, base rates stratified by site and object class', 'predict_test334')
    DL, tr, te = data()
    ...
    rd.say('- %d objects in %d strata; %d units, FDR %.1f%%; coverage %.2f%% (without Bull1 units %.2f%%); current 18.99%% / 6.70%%.' % (len(both), len({s for t, m, s in both}), real, 100 * fdr, 100 * c, 100 * cnb))
    rd.say()
    rd.rec('SX1', 'stratified FDR <= 10%', '%.1f%%' % (100 * fdr), fdr <= 0.10)
    rd.rec('SX2', 'stratified coverage at least 18.99%', '%.2f%%' % (100 * c), c >= 0.1899)
    rd.rec('SX3', 'stratified coverage without Bull1 units at least 6.70%', '%.2f%%' % (100 * cnb), cnb >= 0.0670)
    rd.rec('SX4', 'progress rule: none (validation; lower stratified figures replace the current ones)', 'SX2 %s, SX3 %s' % (c >= 0.1899, cnb >= 0.0670), False)
    rd.finish()
