"""Two-hundred-and-fifty-first registered prediction set (PREDICTIONS.md, TR1-TR3): decipherment loop 76, SINGLE-DOUBLE and
550-END (grammar.parse7). Writes results/predict_test251.md."""
...
def main():
    ...
    rd = R.Round('Two-hundred-and-fifty-first registered predictions: decipherment loop 76, two small rules', 'predict_test251')
    DL, tr, te = data()
    ...
    f0 = lambda t: G.parse6(t, H, hc, mc, labels, low) is not None
    f1 = lambda t: G.parse7(t, H, hc, mc, labels, low) is not None
    ...
    rd.rec('TR1', 'B margin rises, real more than shuffled', 'margin %.2f -> %.2f; real %+.2f, shuffled %+.2f points' % (100 * b0, 100 * b1, 100 * (r1 - r0), 100 * (s1 - s0)), ok1)
    f0m, f1m = G.margin(DF, f0), G.margin(DF, f1)
    rd.rec('TR2', 'F margin rises', '%.2f -> %.2f' % (100 * f0m, 100 * f1m), f1m > f0m)
    rd.rec('TR3', 'progress rule', 'TR1 %s' % ok1, ok1)
    rd.finish()
