"""Two-hundred-and-ninety-sixth registered prediction set (PREDICTIONS.md, VW1-VW3): decipherment loop 121, the picture
vault of set 294 (all unit kinds, 2+ objects, 67%+) with weighted votes: each qualifying unit votes with weight
share * log2(1 + objects), its picture's share among the other objects times the log of their number. Scored by the
excess of correct predictions over the shuffle median (set 295's lesson). Writes results/predict_test296.md."""
import math
import random
...
def vault(objs):
    ...
    for t, m in objs:
        ...
        for g in grams(t):
            ...
            if lab:
                sh = sum(x == lab for u, x in rest) / len(rest)
                votes[lab] += sh * math.log2(1 + len(rest))
        if votes:
            n += 1
...
def main():
    ...
    rd = R.Round('Two-hundred-and-ninety-sixth registered predictions: decipherment loop 121, the picture vault with weighted votes', 'predict_test296')
    P = X.pools(F, recs)
    real = [vault(both) for clean, both in P.values()]
    h, n = sum(a for a, b in real), sum(b for a, b in real)
    rnd = random.Random(296)
    null = []
    for _ in range(200):
        tot = 0
