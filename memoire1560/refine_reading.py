"""Apply explicitly reviewed line readings; never overwrite inherited transcription."""
...
rows={}
for fn in ['reading_152r-154r.txt','reading_154v-156v.txt']:
    for line in (ROOT/fn).read_text(encoding='utf8').splitlines():
        if line and not line.startswith('#'):
            label,value=line.split('\t',1)
            rows[label]=value
rows.update(updates)
head='# Working normalized reading: all ten pages, marginal addition, and cipher endorsement.\n# Brackets retain uncertainty; this is not yet a fully audited final edition.\n'
(ROOT/'reading_working.tsv').write_text(head+'\n'.join(k+'\t'+rows[k] for k in sorted(rows))+'\n',encoding='utf8')
(ROOT/'reading_revisions.json').write_text(json.dumps(updates,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
print(len(rows),'manuscript lines; remaining bracketed passages:')
