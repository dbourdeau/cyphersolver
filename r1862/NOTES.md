...
**Result: read in part (~84% of the code groups).** The cipher is broken and the key rebuilt; the letter is read
continuously on pages 1, 3 and 4 and on the first half of page 2. One block — page 2, lines 18–28 — is
transcribed but not yet read.
...
6442 suo, 6240 ave, 5373 part, 6477 poc, 6287 non, 6479 tio, 5347 papa, 6247 Cesare. 698/5198/5199 stand at the
edges of the ciphered passages and appear to be paragraph marks (R1874 uses 6499 the same way).
...
- 20 code-group tokens (1.6%): 6274 x2, 6478 x2, 5386 x2, 5140, 5174, 5378, 5379, 6278, 6281, 6288, 6241, 683, 699, 5349, 5370, 5389, 51708 - blocker: open-codes; they sit in the word/name band of the key, which R1874's decipherment does not cover
- page 2, lines 18-28 (about 190 groups, ~15%) - blocker: open-codes; transcribed and largely image-true, but split boundaries and 5/2, 3/7, 6/0/8 confusions at the limit of this scan leave the block unread
...
- [x] siblings: R1874 (Busta 30) opened; it carries the contemporary decipherment that gave the key, and its second enciphered copy gave more alignment data
- [x] clear-pages: this record has none; R1874's images 8955-8959 are the decipherment and were transcribed here
- [x] known-keys: no Venetian key of this series on DECODE or in print fits; the key was rebuilt instead
...
- [x] key-rebuild: soft-EM alignment of R1874's cipher to its decipherment, with a synthetic control; grid cells then filled by structure and word codes read from context
- [x] retry: every group re-run with the extended key after each of five image passes; the session hit its rate limit with page 2 lines 18-28 still open
Work stopped when the session hit its rate limit, not because the remaining gaps are closed.
