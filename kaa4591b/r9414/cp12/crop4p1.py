# P1 per-line crops l.19-39 (upright), 4 bands at 2x, drift-following as crop4p3.py
import sys, numpy as np
from PIL import Image, ImageOps, ImageDraw
im=Image.open('../img/IMG_R9414_I44519_P1.jpg').convert('L')
a=np.asarray(im).astype(float); Q=[500,1230,1960,2690,3420]; Y0,Y1=1950,4500
def peaks(x0,x1):
...
print(P)
y19=int(sys.argv[1]) if len(sys.argv)>1 else 2160
A=[p for p in P[0] if p>=y19-50]
OFF=[0,-45,-95,-128]  # upward slope: line rises ~1 line across the page
C=[[min(P[b],key=lambda p:abs(p-A[0]-OFF[b])) for b in range(4)]]
for k in range(1,21):
    row=[]
    pitch=(A[k]-A[k-1]) if k<len(A) else 108
    for b in range(4):
...
for k in range(21):
    ...
    for b in range(4):
        ...
        dr=ImageDraw.Draw(s); dr.rectangle((0,0,50,20),fill='white'); dr.text((4,4),f'{k+19:02d}{"abcd"[b]}',fill='red')
        strips.append(s)
    ...
    out.save(f'cp12/p1h/P1_{k+19:02d}.png')
for k in range(21): print(k+19,C[k])
