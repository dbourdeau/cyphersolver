...
for mask in range(1,64):
 ...
 for restart in range(4):
  ...
  for it in range(12000):
   t=int(rng.integers(n));u=int(rng.integers(len(key)));old=key[t];new=key[u];key[t]=new;key[u]=old;p=np.union1d(positions[t],positions[u] if u<n else np.array([],dtype=int));x=key[grams[p]];iv=((x[:,0]*m.A+x[:,1])*m.A+x[:,2])*m.A+x[:,3];nv=m.lp[iv];delta=float(nv.sum()-vals[p].sum());temp=2*(1-it/12000)+.15
 ...
 (root/'segmentation_swap_results.json').write_text(json.dumps(sorted(results,key=lambda r:r['score'],reverse=True),indent=2))
