...
class Segment {
 ...
 var seeds=new Dictionary<string,char>{{"33",'m'},{"02",'a'},{"13",'i'},{"45",'s'},{"04",'a'}};
 ...
 for(int restart=0;restart<100;restart++){int N=Math.Max(A,n);var k=new int[N];string extra="esantirulodcmp";for(int i=0;i<N;i++)k[i]=i<A?i:alpha.IndexOf(extra[(i-A)%extra.Length]);for(int i=N-1;i>0;i--){int j=rng.Next(i+1),t=k[i];k[i]=k[j];k[j]=t;}foreach(var kv in seeds)if(vocab.Contains(kv.Key))k[vocab.IndexOf(kv.Key)]=alpha.IndexOf(kv.Value);double score=Score(k,grams);
