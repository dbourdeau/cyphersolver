...
results=[]
for mask in range(6):
 prefixes={str(mask)};seqs=[];bad=0
 for r in runs:
  seq=[]
  for part in (r['digits'].split('.') if mask<3 else [r['digits'].replace('.','')]):
   if mask%3==1:part=part.replace('6','')
   if mask%3==2:part=part.replace('6','60')
   if not part:continue
   if len(part)%2:bad+=1
   seq.extend(part[i:i+2] for i in range(0,len(part),2))
  seqs.append(seq)
 vocab=sorted(set(sum(seqs,[])));n=len(vocab)
 if not 17<=n<=100:continue
 ...
 for restart in range(15):
  key=np.array((list(range(m.A))+[m.index[c] for c in "esantirulodcmp"]*6)[:max(n,m.A)]);rng.shuffle(key);x=key[grams];inds=((x[:,0]*m.A+x[:,1])*m.A+x[:,2])*m.A+x[:,3];vals=m.lp[inds];score=vals.sum()
  for it in range(30000):
   t=int(rng.integers(n));u=int(rng.integers(len(key)));old=key[t];new=key[u];key[t]=new;key[u]=old;p=np.union1d(positions[t],positions[u] if u<n else np.array([],dtype=int));x=key[grams[p]];iv=((x[:,0]*m.A+x[:,1])*m.A+x[:,2])*m.A+x[:,3];nv=m.lp[iv];delta=float(nv.sum()-vals[p].sum());temp=2*(1-it/30000)+.15
   if delta>=0 or rng.random()<math.exp(max(-700,delta/temp)):
    vals[p]=nv;score+=delta
    if score>best:best=score;bk=key.copy()
   else:key[t]=old;key[u]=new
 ...
 (root/'pair_results.json').write_text(json.dumps(sorted(results,key=lambda r:r['score'],reverse=True),indent=2))
