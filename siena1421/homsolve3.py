"""Homophonic solver with optional null signs (nulls=1): a sign may map to NULL and is then dropped.
...
NULLS = o.get('nulls', '0') == '1'; NULLID = A  # sentinel
NPEN = float(o.get('npen', 3.0))
choices = letters + ([NULLID] * max(1, len(letters)//6) if NULLS else [])
fixed = {}
...
def make_scorer(Mk):
    ...
    remap = np.array([Mk.index[c] for c in M.alpha] + [0])
    def score(key):
        full = np.concatenate([key, [M.index[' '] if WORDS else 0, 0]])
        yy = full[x]
        keepm = yy != NULLID
        xb = x[keepm]; yy = yy[keepm]
        y = remap[yy]
        n = len(y) - k + 1
        ...
        ok = np.ones(n, bool)
        if not WORDS:
            br = np.where(xb == BREAK)[0]
            for b in br: ok[max(0, b - k + 1):b + 1] = False
        s = lp[ctx][ok].sum()
        nn = cnt[key == NULLID].sum() if NULLS else 0.0
        kk = np.where(key == NULLID, 0, key); w = np.where(key == NULLID, 0, cnt)
        Nn = max(1.0, w.sum())
        ob = np.bincount(kk, weights=w, minlength=A) / Nn; m = ob > 0
        return s - KLW * Nn * float((ob[m] * np.log(ob[m] / freq[m])).sum()) - NPEN * nn
    return score
...
def ch(k): return '_' if k == NULLID else M.alpha[k]


def show(key):
    if WORDS: return ' '.join(''.join(ch(key[si[s]]) for s in w) for w in units)
    return ' | '.join(''.join(ch(key[si[s]]) for s in r) for r in runs)
...
for r in range(R):
    ...
    for sc in scorers:
        ...
        for it in range(IT):
            if rnd.random() < 0.8:
                i = rnd.choice(free); old = key[i]; key[i] = rnd.choice(choices)
                s = sc(key)
...
for sc_, key in res[:int(o.get('top', 1))]:
    ...
    print(' '.join('%s=%s' % (s, ch(key[si[s]])) for s in signs))
    print(show(key))
