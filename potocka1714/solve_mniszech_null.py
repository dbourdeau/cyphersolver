...
for line in [(p/'r7524-cipher.txt').read_text().replace('\n',' ')]:
 ...
 for t in re.findall(r'\[[^]]*\]|\d+',line):
  if t.startswith('['):row.extend(-m.index[c]-1 for c in lm.norm(t,spaces=False))
   ...
   if n==33:continue
   if n==130:
    row.extend([-m.index['c']-1,-m.index['h']-1]);continue
...
best=-1e20;rng=random.Random(19)
crib={22:'h',118:'e',82:'t',36:'m',31:'a',81:'n',17:'o',68:'w'}
locked={symbols.index(n):m.index[c] for n,c in crib.items()}
mutable=[i for i in range(len(symbols)) if i not in locked]
for run in range(160):
 k=np.array([locked.get(i,rng.randrange(m.A)) for i in range(len(symbols))]);cur=score(k)
 for it in range(50000):
  a=rng.choice(mutable);old=k[a];k[a]=rng.randrange(m.A)
